...
namespace CD_Test
{
    public class FileReaderTests
    {
        ...
        public async Task ReadDigitsInBlocksAsync_ReadsLinesOfZeroAndOne_Correctly()
        {
            ...
            Assert.Equal(new byte[] { 0, 1, 0 }, resultBlocks[0].Data?.Take(resultBlocks[0].LastElementIndex + 1));
            Assert.Equal(new byte[] { 1, 0, 1 }, resultBlocks[1].Data?.Take(resultBlocks[1].LastElementIndex + 1));
            Assert.Equal(new byte[] { 0 }, resultBlocks[2].Data?.Take(resultBlocks[2].LastElementIndex + 1));
        }
